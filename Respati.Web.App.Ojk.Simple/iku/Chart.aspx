<%@ Page Title="" Language="C#" MasterPageFile="~/Simple.Master" AutoEventWireup="true" CodeBehind="Chart.aspx.cs" Inherits="Respati.Web.App.Ojk.Simple.iku.Chart" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="FootContent" runat="server">
    <script>
        $(".rocItem").on("click", function () {
            ...
            var fullUrl = url + "?NIP=" + nd.text().trim() + "&VW=IKI&ID=<%=Request.QueryString["id"] %>";
            var oWnd = $find("<%=RadWindow1.ClientID%>");
            oWnd.setUrl(fullUrl);
            oWnd.setSize(500, 400);

            var offs = $(".RadOrgChart").parent().offset();
            var width = $(".RadOrgChart").width();
            var left = (width - 500) / 2;

            oWnd.moveTo(offs.left + left, offs.top);
            //oWnd.moveTo(offs.left + left, 0);
            oWnd.show();
        });
    </script>
</asp:Content>
